...
struct BenchmarkListTags: AsyncParsableCommand {
    static let configuration = CommandConfiguration(
        commandName: "benchmark-list-tags",
        abstract: "Benchmark list_tags on the plugin transport.",
        aliases: ["benchmark_list_tags"]
    )
    ...
    func run() async throws {
        ...
        let scenarios = listTagBenchmarkScenarios()
        ...
        while Date() < deadline {
            let scenario = scenarios[callIndex % scenarios.count]
            let event = try await listTagBenchCall(
                scenario: scenario,
                phase: "measured",
                service: service,
                intervalMS: intervalMS,
        ...
        let summary = renderListTagSummary(
            startedAt: startedAt,
            endedAt: Date(),
            scenarios: scenarios,
            stats: stats
...
private struct ListTagBenchmarkScenario {
    let name: String
    let statusFilter: String?
    let includeTaskCounts: Bool
}

private struct ListTagBenchEvent: Codable {
...
private func listTagBenchmarkScenarios() -> [ListTagBenchmarkScenario] {
    [
        ListTagBenchmarkScenario(name: "active_no_counts", statusFilter: "active", includeTaskCounts: false),
        ListTagBenchmarkScenario(name: "active_with_counts", statusFilter: "active", includeTaskCounts: true)
    ]
}

private func listTagBenchCall(
    scenario: ListTagBenchmarkScenario,
    phase: String,
    ...
    callIndex: inout Int,
    rawURL: URL
) async throws -> ListTagBenchEvent {
    callIndex += 1
    let started = Date()
    do {
        let page = try await service.listTags(page: PageRequest(limit: 50), statusFilter: scenario.statusFilter, includeTaskCounts: scenario.includeTaskCounts)
        let elapsed = Date().timeIntervalSince(started) * 1000
        let event = ListTagBenchEvent(
...
private func renderListTagSummary(
    startedAt: Date,
    endedAt: Date,
    scenarios: [ListTagBenchmarkScenario],
    stats: [String: BenchmarkStatsAccumulator]
) -> String {
    var lines: [String] = []
    lines.append("# list_tags Benchmark Summary")
    lines.append("")
